...
export const fetchMangaList = async (req, res) => {
  try {
    ...
    if (query.includedTags) {
      const tags = Array.isArray(query.includedTags)
        ? query.includedTags
        : [query.includedTags];
      tags.forEach((tag) => {
      ...
      response.data.data.map(async (manga) => {
        ...
        if (authorRel) {
          try {
            ...
            console.error(
              `Error fetching author for manga ID ${manga.id}: ${err.message}`
            );
            return null;
          }
        }
      ...
      response.data.data.map(async (manga) => {
        ...
        if (artistRel) {
          try {
            ...
            console.error(
              `Error fetching artist for manga ID ${manga.id}: ${err.message}`
            );
            return null;
          }
        }
...
export const fetchMangaById = async (req, res) => {
  ...
  try {
    ...
    const MangaTitle = manga.attributes?.title
    ...
    if (coverRel) {
      try {
        ...
        console.error(
          `Error fetching cover for manga ID ${id}: ${err.message}`
        );
      }
    }
    ...
    if (authorRel) {
      try {
        ...
        console.error(
          `Error fetching author for manga ID ${id}: ${err.message}`
        );
      }
    }
    ...
    if (artistRel) {
      try {
        ...
        console.error(
          `Error fetching artist for manga ID ${id}: ${err.message}`
        );
      }
    }
...
export const fetchVolumeListByID = async (req, res) => {
  ...
  try {
    ...
    const VolumeListData = Object.entries(volumesObj).map(
      ([volumeNumber, volumeData]) => {
...
export const addManga = async (req, res) => {
  try {
    ...
    const [existing] = await db.execute("SELECT * FROM Manga WHERE title = ?", [
      title,
    ]);
