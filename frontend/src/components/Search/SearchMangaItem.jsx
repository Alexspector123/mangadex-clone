...
const SearchMangaItem = ({ data }) => {
    ...
    const { mangaData, isLoading, error } = useFetchByID(data.id);
    ...
    return (
        <div>
            {!isLoading && (
                ...
                className=''
                key={mangaData.id}
                href={`/titles/${mangaData.id}`}>
                    <div className='grid grid-cols-[56px_1fr] gap-2 w-full
                                    min-h-20
                                    bg-slate-100 hover:bg-slate-300
                                    transition-all duration-200
                                    p-1.5'>
                        <div className='h-full w-full'>
                            <img className='rounded shadow-md w-full h-full object-fill' src={mangaData.Cover} alt="Cover img" />
                        </div>
