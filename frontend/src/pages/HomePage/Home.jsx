...
const Home = () => {
  ...
  const { mangaData, error } = useFetchList(queryParams);
  const {
    mangaIDs,
    mangaTitles,
    coverUrls,
  ...
  return (
    <div className="relative h-[324px] md:h-[440px]">
              ...
              <div className="relative">
                ...
                <div className="absolute bottom-0 left-0 right-0 h-full flex justify-center md:items-center">
                  <div className="p-4 mb-6 md:mb-0 md:py-4 md:px-6 grid grid-rows-[1fr_1rem] md:grid-rows-1 gap-2 md:h-[77%] sm:h-[65%] h-[70%] mt-auto xl:max-w-[1440px] w-full mx-auto">
                    <SlideCard
                      mangaAuthors={mangaAuthors[index]}
                      tags={tags[index]}
                      mangaTitles={mangaTitles[index]}
                      mangaDescriptions={mangaDescriptions[index]}
                      coverUrls={coverUrls[index]}
                    />
                  </div>
                </div>
              </div>
      ...
      <div className="absolute bottom-0 right-5 left-5 flex justify-between gap-5 text-xl z-10 md:justify-normal md:left-auto md:bottom-14 lg:text-2xl lg:bottom-4">
        <IoIosArrowBack
          className="cursor-pointer transition-all duration-200 w-8 h-8 p-1 hover:bg-slate-300 hover:rounded-full"
      ...
      <h2 className="absolute top-13 sm:top-15 left-1 z-2 min-[1448px]:left-10 min-[1448px]:text-4xl font-semibold sm:text-2xl text-xl font-spartan px-6">
        Popular New Titles
      </h2>
    </div>
  );
