...
const Latest = () => {
  const { chapters, error } = useFetchChapters(18, "readableAt");
  const { mangaTitle, coverUrl } = chapters || {};
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h2 className="z-2 min-[1448px]:text-4xl font-semibold sm:text-2xl text-xl font-spartan">
          Latest Updates
        </h2>
            ...
            <a href={`/titles/${chap.mangaID}`}>
              ...
              >
                <div className="h-20 w-14 flex-shrink-0">
                  <img
                    className="flex items-start relative mb-auto select-none w-full h-full"
                    src={chap.coverUrl}
                    alt={chap.title}
                  />
                </div>
                <div className="flex flex-grow flex-col justify-evenly">
                  <h6 className="text-base font-bold line-clamp-1 break-all">
                    {chap.mangaTitle}
                  </h6>
                  <p>
                    Ch. {chap.chapter}{" "}
                    {chap.volume ? `Vol. ${chap.volume}` : ""}
                  </p>
                  <p className="text-sm text-gray-600">
                    {chap.group} • {chap.language.toUpperCase()}
                  </p>
