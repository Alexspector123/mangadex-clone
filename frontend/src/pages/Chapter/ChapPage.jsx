...
const ChapPage = () => {
  ...
  const { showSidebar = () => {}, showHeader = true } =
    useOutletContext() || {};

  const { chapterReaderData, error } = useFetchChapterReader(id);
  const {
    chapterData,
  ...
  const { allChapters } = useChapterList(mangaID);
  ...
  const nextChapter = useMemo(() => {
    ...
    const sameGroupAndLanguage = allChapters.filter(
      (chap) =>
        ...
        chap.translatedLanguage === chapterData.translatedLanguage
    );

    if (sameGroupAndLanguage) {
      return sameGroupAndLanguage.find(
        (chap) =>
            ...
            >
              ...
              <div
                className="flex flex-row justify-between items-center gap-2
                    text-base"
              >
                <div className="bg-gray-100 flex-grow text-center p-0.5">
                  Ch. {chapterData.chapterNo}
                </div>
                <div className="bg-gray-100 flex-grow text-center p-0.5">
                  Pg. {currentPage}/{chapterReaderData.length}
                </div>
                <div
